...
function HomeworkCenterContent() {
  ...
  const isLoading = classesLoading || (activeClassId ? assignmentsLoading : false);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Date Picker Bar */}
      <div className="flex flex-wrap items-center justify-between gap-4 border border-violet-100 bg-white p-5 rounded-2xl shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 rounded-xl border border-violet-200 px-3 py-1.5 text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-violet-300 bg-white cursor-pointer"
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {classesLoading ? (
          ...
          <div className="rounded-2xl border-2 border-dashed border-violet-200 bg-violet-50/20 px-6 py-10 text-center">
      ...
      <div className="space-y-4">
        <div className="border-b border-slate-100 pb-2">
          <h3 className="font-montserrat text-lg font-bold text-slate-800 flex items-center gap-2">
            Homework List {activeClassId ? `(${classes.find(c => c.id === activeClassId)?.name || ""}${activeSectionId ? ` - Section ${activeSectionId}` : " - All Sections"})` : ""}
          </h3>
        </div>
  ...
  return (
    <div 
      className={`rounded-2xl border p-5 bg-white shadow-sm transition-all duration-200 ${
        isActive ? "border-violet-400 ring-2 ring-violet-100 shadow-md" : "border-slate-100 hover:border-violet-200 hover:shadow-md"
      }`}
    >
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
              isActive && activeSectionId === ""
                ...
                : "bg-slate-50 text-slate-600 hover:bg-violet-100"
            }`}
          >
            All
          </button>
