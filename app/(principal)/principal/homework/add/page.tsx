...
function AddHomeworkContent() {
  ...
  // Pre-select first class if only 1 is loaded
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setIsScanning(false);
          setScanStep(0);
          toast.success("AI extraction completed! Homework details populated.");
        }, 1200);
      }, 1000);
    }, 800);
  }
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      <div>
        <button
          onClick={() => router.push("/principal/homework")}
      ...
      {/* AI Scanner Card */}
      <div className="rounded-2xl border border-violet-100 bg-gradient-to-r from-violet-50/50 to-indigo-50/30 p-6 shadow-sm relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-10">
          <Sparkles className="h-24 w-24 text-violet-600 animate-pulse" />
        </div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
          ...
          <div>
            <label className="flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-sm font-semibold text-white shadow-md hover:bg-violet-700 active:scale-[0.98] transition-all cursor-pointer">
      ...
      {/* Manual Upload Form */}
      <form onSubmit={handleSubmit} className="rounded-2xl border border-violet-100 bg-white p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              disabled={classesLoading || createMut.isPending}
            ...
            >
              <SelectTrigger className="w-full rounded-xl border-violet-200 bg-white h-[42px] text-sm">
        ...
        <div className="space-y-6">
          {homeworkItems.map((item, idx) => (
            <div key={item.id} className="relative p-5 rounded-2xl border border-violet-100 bg-slate-50/20 space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                <span className="font-bold text-sm text-violet-700">Homework Task #{idx + 1}</span>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white py-2.5 px-4 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition-all resize-none"
                />
              </div>
